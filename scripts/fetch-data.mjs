...
const STRAPI_URL = "https://content.bitriel.app";
...
const cacheConfigs = [
    {
        ...
        additionalProcess: [
            (data, preview_data, config, lang, isProduction) => {
                const { folder } = config;
                const combineData = Object.fromEntries(
                    preview_data.map((c) => [c.slug, c.icon])
                );
                const path = savePath(
                    folder,
                    getFileNameByLang("logo_map.json", lang)
                );
            ...
            (data, preview_data, config, lang, isProduction) => {
                if (data.length > 0 || preview_data.length > 0) {
                    ...
                    for (const dataContent of preview_data) {
                        ...
                        const { folder, content, description, title } =
                            dataContent;
                        const contentSave = {
                            content,
                            description,
                        ...
                        const folderPath = saveFolderChild(
                            folderParent,
                            folder
                        );
                        ...
                        const filePath = saveFileInFolderChild(
                            folderParent,
                            folder,
                            fileName
                        );
                        writeJSONFile(filePath, contentSave).catch(
                            console.error
                        );
                    }
                }
            ...
            async (data, preview_data, config, lang, isProduction) => {
                ...
                if (preview_data.length > 0 || data.length > 0) {
                    ...
                    const assetMap = Object.fromEntries(
                        chains.map((chain) => [chain.slug, chain])
                    );
                    ...
                    const pathDisabledXcmChannels = savePath(
                        `${folder}/disabled-xcm-channels`,
                        `${prefix}.json`
                    );

                    writeJSONFile(path, dataSave).catch(console.error);
                    writeJSONFile(
                        pathDisabledXcmChannels,
                        disabledXcmChannels
                    ).catch(console.error);
                }
            },
...
const savePath = (folder, fileName) =>
    `data/${folder}/${fileName || "list.json"}`;
const saveImagesPath = (folder) => `data/${folder}/images`;
const saveFolderChild = (folderParent, folder) =>
    `data/${folderParent}/${folder}`;
const saveFileInFolderChild = (folderParent, folder, fileName) =>
    `data/${folderParent}/${folder}/${fileName}`;
const urlImage = (folder, field, name) =>
    `${RESOURCE_URL}/${folder}/images/${field}/${name}`;
            ...
            for (const field of fieldsImage) {
                ...
                if (dataField) {
                    ...
                    if (cachedImage) {
                        ...
                        const newFileName =
                            cachedImage ||
                            (await downloadFile(dataField, folderFieldImage));
...
const main = async () => {
    ...
    const configs = folder
        ? cacheConfigs.filter((c) => c.folder === folder)
        : cacheConfigs;
    for (const config of configs) {
        ...
        for (const lang of langs) {
            const dataContent = await fetchAndProcessData(
                getUrl(config.url, false, lang),
                folder,
                downloadDir,
                fieldsImage
            );
            const previewData = await fetchAndProcessData(
                getUrl(config.url, true, lang),
                folder,
                downloadDir,
                fieldsImage
            );
            ...
            if (config.fileName && isProduction && dataContent) {
                const path = savePath(
                    folder,
                    getFileNameByLang(config.fileName, lang)
                );
                await writeJSONFile(path, dataContent);
            }
            if (config.preview && previewData) {
                const previewPath = savePath(
                    folder,
                    getFileNameByLang(config.preview, lang)
                );
